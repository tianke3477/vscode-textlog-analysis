...
export class LargeFileSystemProvider implements vscode.FileSystemProvider { // export only for test access?
	...
	markLimitSize(uri: vscode.Uri, limitSize = true, filters?: { search: string, replace: string }[]) {
		...
		if (!this._uriMap.has(uri.toString())) {
			console.log(`vscode-textlog-analysis.markLimitSize(uri=${uri.toString()})... limitSize=${limitSize}`);
	...
	stat(uri: vscode.Uri): vscode.FileStat {
		...
		// console.log(`vscode-textlog-analysis.stat(uri=${uri.toString()})... _limitSize=${limitSize}`);
	...
	async readFile(uri: vscode.Uri): Promise<Uint8Array> {
		...
		console.log(`vscode-textlog-analysis.readFile(uri=${uri.toString()})... _limitSize=${curSet.limitSize} filters=${JSON.stringify(curSet.filters)}`);
	...
	watch(uri: vscode.Uri): vscode.Disposable {
		console.log(`vscode-textlog-analysis.watch(uri=${uri.toString()}...`);
		return new vscode.Disposable(() => {
			console.log(`vscode-textlog-analysis.watch.Dispose ${uri}`);
			let curSet = this._uriMap.get(uri.toString());
			if (curSet) {
	...
	onDidCloseTextDocument(uri: vscode.Uri) {
		// console.log(`vscode-textlog-analysis onDidCloseTextDocument(${uri.toString()})...`);
		let curSet = this._uriMap.get(uri.toString());
		if (curSet) {
	...
	readDirectory(uri: vscode.Uri): [string, vscode.FileType][] {
		console.log(`vscode-textlog-analysis.readDirectory(uri=${uri.toString()}...`);
		let entries: [string, vscode.FileType][] = [];
		// list all dirs and files matching our filter:
		const dirEnts = fs.readdirSync(uri.fsPath, { withFileTypes: true });
		for (var i = 0; i < dirEnts.length; ++i) {
			// console.log(` vscode-textlog-analysis.readDirectory found ${dirEnts[i].name}`);
	...
	writeFile(uri: vscode.Uri, content: Uint8Array, options: { create: boolean, overwrite: boolean }): void {
		console.log(`vscode-textlog-analysis.writeFile(uri=${uri.toString()}...`);
		throw vscode.FileSystemError.NoPermissions();
	}

	rename(oldUri: vscode.Uri, newUri: vscode.Uri, options: { overwrite: boolean }): void {
		console.log(`vscode-textlog-analysis.rename(oldUri=${oldUri.toString()}...`);
		throw vscode.FileSystemError.NoPermissions();
	}

	delete(uri: vscode.Uri): void {
		console.log(`vscode-textlog-analysis.delete(uri=${uri.toString()}...`);
		throw vscode.FileSystemError.NoPermissions();
	}

	createDirectory(uri: vscode.Uri): void {
		console.log(`vscode-textlog-analysis.createDirectory(uri=${uri.toString()}...`);
		throw vscode.FileSystemError.NoPermissions();
	}
